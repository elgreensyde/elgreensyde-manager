...
- [ ] **The "Fat Finger" Test:** Open the app on your phone. Tap buttons quickly. Ensure the UI doesn't accidentally zoom in or highlight blue text (verifying our `select-none` armor).
- [ ] **The Global Dialog Test:** Go to any module (e.g., Inventory or Tasks). Attempt to delete a record. Verify the new custom `GlobalDialog` slides up properly and actually deletes the item when confirmed.
- [ ] **Scrolling Test:** Open long pages (like `Batches` or `Tasks`). Ensure the screen scrolls smoothly and that the bottom navigation bar doesn't cover up the last item on the list.
- [ ] **Smart Assistant Overlay:** Tap the floating emerald AI button. Ensure it opens the context-aware protocol sheet. Navigate to `/batches` and click it again—does it shift to the "Land Preparation" protocols?

---

## 👨‍🌾 Phase 2: The Core Agronomic Loop (The Brain)

- [ ] **Weather & Thresholds:** Open the `More > Weather` dashboard. Verify that the current VPD and Disease Radar numbers populate instead of showing "Loading...". 
...
- [ ] **Nursery Creation:** Add a new tray for "Sweet Basil". Verify that the system automatically assigns exactly a 14-day germination/target transplant offset.
- [ ] **The Transplant Flow:** Click the "Transplant & Assign" button on a Ready tray. Assign it to an empty Plot. Ensure the tray moves to "Completed" and the Plot becomes "Active" with the correct crop.
- [ ] **The Fusarium Guard (CRITICAL):**
...
1. **What did you tap?** (e.g., *I pressed complete on a Sweet Basil harvest task*)
2. **What did you expect to happen?** (e.g., *I expected it to disappear*)
3. **What actually happened?** (e.g., *The screen froze, or it duplicated the task*)
